...
Proof.
unfold hoare_triple.
  intros P b st r H H1.
  inversion H; subst.
  - exists st. split; try reflexivity. destruct H1. assumption.
  - exists st. destruct H1. rewrite H1 in H2. discriminate.
Qed.
...
(*the program must start in a state where if b is true, then P is also true*)
...
Theorem hoare_assume : forall (P : Assertion) (b : bexp),
 ...
 Proof.
  ...
  intros P b st r H H1.
  inversion H; subst.
  - exists st. split; try reflexivity. apply H1. apply H2.
Qed.
...
(* ================================================================= *)
 ...
 Theorem hoare_choice' : forall Px Qx Py Qy c1 c2,
{{ Px }} c1 {{ Qx }} ->
  {{ Py }} c2 {{ Qy }} ->
  {{ Px /\ Py}} <{ c1 !! c2 }> {{ Qx  \/ Qy }}.
Proof.
  ...
  - exists s. split.
  -- reflexivity.
  --  inversion H1; subst. left. specialize (H st (RNormal s)). destruct H.
  --- assumption.
  --- destruct H2. apply H.
  --- destruct H. inversion H. apply H3.
  ---  right. specialize (H0 st (RNormal s)). destruct H0.
  ---- assumption.
  ---- destruct H2. apply H2.
  ---- destruct H0. subst. inversion H0. apply H3.
  - exists st. inversion H1; subst.
  --  specialize (H st (RError)). destruct H.
  ---  apply H7.
  --- destruct H2. apply H.
  --- inversion H. contradict H. discriminate.
  --  specialize (H0 st (RError )). destruct H0; subst.
        --- apply H7.
        --- destruct H2. apply H2.
        --- destruct H0. contradict H0. discriminate.
Qed.
...
Example assert_assume_example:
  ...
  {{ X = 42 }}.   
Proof.
  apply hoare_seq with (Q := fun st => st X = 2 /\ st X = 1).
  - unfold hoare_triple. intros. exists st. inversion H; subst.  split.
    --  inversion H0. rewrite H1 in H0. destruct H0. discriminate H3.
    --  simpl.  destruct H0. rewrite H0 in H1. discriminate H1.
  -  unfold hoare_triple. intros. exists st. inversion H; subst. split.
    -- reflexivity.
    -- split; auto. inversion H2. rewrite Nat.eqb_eq in H3. assumption. 
Qed.

(*Obs: Nat.eqb_eq relates the equality comparison operator =? 
and the equality operator = for natural numbers, solving the divergence in H3*)

(* The example shows that if the program is started in a state satisfying the assertion
X = 1, and the assumption X = 2 holds true, then after executing the assignment
 X := X + 1, if it terminates, the final state will be X = 42.

It shows the use of Hoare Logic to prove the correctness of a non-deterministic
program that includes sequential composition, the command assume and an assignment*)
...
Inductive cstep : (com * result)  -> (com * result) -> Prop :=
  ...
  | CS_E_NonDetChoice1 : forall st c1 c2,
     <{ c1 !! c2 }> / st  --> c1 / st
  | CS_E_NonDetChoice2 : forall st c1 c2,
     <{ c1 !! c2 }> / st  --> c2 / st
...
Proof.
  ...
  --- simpl.  eapply multi_step. apply CS_SeqStep. apply CS_AssumeTrue.   eapply multi_step. apply CS_SeqFinish.
   eapply multi_step. apply CS_SeqStep. apply CS_E_NonDetChoice1.  eapply multi_step. apply CS_SeqStep. 
  apply CS_AssStep. apply AS_Plus1. apply AS_Id.
   eapply multi_step. apply CS_SeqStep. 
  ...
  eapply multi_step. apply CS_AssertStep. apply BS_Eq1. apply AS_Id.
   eapply multi_step. apply CS_AssertStep. apply BS_Eq. simpl. eapply multi_step.  apply CS_AssertTrue.
...
Proof.
intros st a a' H. 
induction H; try reflexivity; simpl; lia.
Qed.
...
Notation "'assert' b {{ P }}"
  := (DCAssert b P)
     (in custom com at level 0, b custom com at level 85, P constr) : dcom_scope.
Notation "'assume' b {{ P }}"
  := (DCAssume b P)
     (in custom com at level 0, b custom com at level 85, P constr) : dcom_scope.
Notation "d '!!' d'"
...
Proof.
 ...
 - (* Assert *) 
    eapply hoare_consequence_pre. 
    -- apply hoare_assert.    
      --  intros st' H1. simpl. split. 
        ---   apply H. assumption. (*split; try assumption.  destruct (beval st' b) eqn:Heq; try reflexivity.  admit.*)
        --- apply H. assumption. (*destruct (beval st' b) eqn:Heq. try reflexivity. admit.*)
 - (* Assume *)  
    eapply hoare_consequence_pre.
    -- apply hoare_assume. 
    -- intros st' HP H1. apply H. split; try apply HP. apply H1. 
- (* DCNonDetChoice *)
    destruct H as [H1 H2].
    ...
    eapply hoare_choice'.
      ---  apply IHd1. apply H1. 
      --- apply IHd2. apply H2.
      --- intros st' H3. simpl. split; try apply H3.
      --- intros st' H3. apply H3.
Qed.
